...
class WalletTx(db.Model):
    ...
    type = db.Column(db.String(20))  # deposit, purchase, commission, refund, transfer_in, cash_out_request
...
def create_wallet_for_user(user):
    """Ensures a user has a wallet record."""
    if not user.wallet:
...
def receive_wallet_tx(wallet, amount, note="", reference=""):
    """Credits a wallet for incoming transfers (P2P)."""
    wallet.balance = Decimal(wallet.balance) + Decimal(amount)
    wallet.updated_at = datetime.utcnow()
...
def deduct_wallet(wallet, amount, note="", reference=""):
    """Debits a wallet (e.g., for purchases/transfers). Records transaction as a negative amount."""
    if Decimal(wallet.balance) < Decimal(amount):
        raise ValueError("Insufficient balance")
    # Note: amount passed here must be positive. We store it as negative in the tx.
    wallet.balance = Decimal(wallet.balance) - Decimal(amount)
    wallet.updated_at = datetime.utcnow()
    # Using type 'purchase' for general deductions, even if for a transfer.
...
def home():
    if current_user.is_authenticated:
        ...
        # NOTE: You need to create 'dashboard.html'
        return render_template('dashboard.html', user=current_user)
    # NOTE: You need to create 'home.html'
...
def register():
    ...
    # NOTE: You need to create 'register.html'
...
def login():
    ...
    # NOTE: You need to create 'login.html'
...
def wallet_view():
    ...
    # NOTE: You need to create 'wallet.html'
...
def mpesa_callback():
    ...
    try:
        ...
        if result_code == 0:
            ...
            # NOTE: If we don't find a pending transaction, we should probably log the successful M-Pesa transaction
            # but currently there is no way to credit the wallet without a matching pending transaction.
        else:
...
def admin_users():
    ...
    # NOTE: You need to create 'admin_users.html'
...
def buy_airtime():
    if request.method == 'POST':
        ...
        try:
            amount = Decimal(amount) # Use Decimal for precision
            if amount <= 0:
                raise ValueError()
        except:
    ...
    # NOTE: You need to create 'buy_airtime.html'
...
def pay_bill():
    ...
    # NOTE: You need to create 'pay_bill.html'
...
def buy_data():
    ...
    # NOTE: You need to create 'buy_data.html'
...
def transfer_funds():
    ...
    # NOTE: You need to create 'transfer_funds.html'
...
def pay_government_services():
    if request.method == 'POST':
        bill_ref = request.form.get('bill_reference', '').strip()
        amount_raw = request.form.get('amount', '').strip()
        try:
            amount = Decimal(amount_raw)
            if amount <= 0: raise ValueError()
        except:
            flash("Enter a valid positive amount.", "danger"); return redirect(url_for('pay_government_services'))
        try:
            deduct_wallet(current_user.wallet, amount, note=f"Govt Payment: {bill_ref}")
            flash(f"Government service payment of KSh {amount} for ref {bill_ref} initiated.", "success")
        except ValueError as e:
            flash(str(e), "danger")
        return redirect(url_for('wallet_view'))
    # NOTE: You need to create 'pay_government_services.html'
    return render_template('pay_government_services.html', user=current_user)

@app.route('/cash_out', methods=['GET', 'POST'])
@login_required
def cash_out():
    # For simplicity, this route handles the client's request to withdraw funds
    if request.method == 'POST':
        withdrawal_amount_raw = request.form.get('withdrawal_amount', '').strip()
        agent_phone = request.form.get('agent_phone', '').strip()
        service_fee = Decimal('10.00') # Fixed fee for cash out service

        try:
            amount = Decimal(withdrawal_amount_raw)
            total_deduction = amount + service_fee
            if amount <= 0: raise ValueError()
        except:
            flash("Enter a valid positive amount.", "danger"); return redirect(url_for('cash_out'))

        try:
            # Deduct the total amount (withdrawal + fee)
            deduct_wallet(current_user.wallet, total_deduction, note=f"Cash Out Request of KSh {amount} (Fee KSh {service_fee}) via Agent {agent_phone}", reference=f"COUT-{datetime.now().timestamp()}")
            flash(f"Cash out request of KSh {amount} sent to agent {agent_phone}. KSh {service_fee} fee deducted.", "success")
        except ValueError as e:
            flash(str(e), "danger")
        return redirect(url_for('wallet_view'))
        
    # NOTE: You need to create 'cash_out.html'
    return render_template('cash_out.html', user=current_user)

# ----------------- NEW Agent Service Routes (Fixing BuildError) -----------------

@app.route('/manage_gov_payments', methods=['GET', 'POST'])
@login_required
def manage_gov_payments():
    if current_user.role != 'agent':
        flash("Access denied", "danger"); return redirect(url_for('home'))

    # Placeholder logic for managing government payment services/commissions
    if request.method == 'POST':
        flash("Government payment settings updated.", "success")
        return redirect(url_for('manage_services')) # Redirect to general management page

    # NOTE: You need to create 'manage_gov_payments.html'
    return render_template('manage_gov_payments.html', user=current_user)

@app.route('/manage_cash_out', methods=['GET', 'POST'])
@login_required
def manage_cash_out():
    if current_user.role != 'agent':
        flash("Access denied", "danger"); return redirect(url_for('home'))

    # Placeholder logic for managing cash out requests
    # In a real app, this would show pending cash out requests from clients
    if request.method == 'POST':
        flash("Cash out management settings updated.", "success")
        return redirect(url_for('manage_services')) # Redirect to general management page

    # NOTE: You need to create 'manage_cash_out.html'
    return render_template('manage_cash_out.html', user=current_user)

# ----------------- Existing Agent Service Route -----------------
...
def manage_services():
    ...
    # NOTE: You need to create 'manage_services.html'
